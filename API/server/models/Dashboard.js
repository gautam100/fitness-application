...
exports.getDashboardContent = function (where, callback) {
    ...
    var sql_query = 'SELECT cat.id, cat.name, pp.name,pp.description,pb.id,pb.product_id,pb.branch_address, pb.branch_city, pb.branch_state, pb.branch_pincode, pb.branch_contact_person, pb.branch_contact_no, pb.monthly_subscription_amt,bi.branch_image,count(distinct bo.id ) as offerCount FROM `product_branches` pb LEFT JOIN branch_images bi on pb.`id`=bi.branch_id LEFT JOIN branch_offers bo on bo.branch_id=pb.`id` LEFT JOIN products pp on pp.id=pb.product_id left join categories as cat on cat.id=pp.category_id WHERE pb.active=1 ';
    ...
    if (where.brand_sort != "") {
        sql_query += ' AND pp.id = ' + where.brand_sort;
    }
...
exports.getBrandNameLists = function (where, callback) {
    var sql_query = 'SELECT products.id, products.name FROM `products` left join categories cat on cat.id=products.category_id WHERE 1=1 ';

    if (where.catId != "") {
        sql_query += ' AND cat.id=' + where.catId;
    }

    sql_query += ' and products.active=1';
    console.log(sql_query);
